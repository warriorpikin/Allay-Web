...
import { getWaitlist } from '../../services/adminApi'
...
export default function WaitlistManager() {
  ...
  return <>
    <div className="admin-page-heading">
      <div><span className="eyebrow">Pre-launch audience</span><h1>Waitlist</h1><p>Manage signups and service interests ahead of launch.</p></div>
      <div className="admin-heading-actions">
        <Button variant="outline" onClick={exportCsv}>Export CSV</Button>
        <Button onClick={() => toast('Discount code generation is not yet connected.', { icon: '✦' })}>Generate discounts</Button>
      </div>
    </div>
    <section className="admin-panel">
      {loading ? <Loader label="Loading waitlist" /> : !entries.length ? <EmptyState title="No waitlist signups yet" message="Signups from the public waitlist form will appear here." /> : <div className="table-wrap"><table><thead><tr><th>Email</th><th>Name</th><th>Phone</th><th>Services</th><th>Joined</th></tr></thead><tbody>{entries.map((entry) => <tr key={entry.id}><td>{entry.email}</td><td>{entry.fullName || '—'}</td><td>{entry.phone || '—'}</td><td>{entry.services.map((service) => service.name).join(', ') || '—'}</td><td>{new Date(entry.createdAt).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}</td></tr>)}</tbody></table></div>}
    </section>
  </>
}
