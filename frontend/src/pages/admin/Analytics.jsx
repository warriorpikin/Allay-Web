...
import { getAdminAnalyticsOverview } from '../../services/adminApi'
...
function Comparison({ item, type = 'number' }) {
  ...
  return <><strong>{display}</strong><small className={`analytics-change analytics-change--${direction}`}>{item.changePercent > 0 ? '+' : ''}{formatPercent(item.changePercent)} vs previous</small></>
}

function SimpleBars({ items = [], labelKey = 'label', valueKey = 'value' }) {
  ...
  return <div className="analytics-bars">{items.map((item) => {
    ...
    return <div key={`${item[labelKey]}-${value}`} className="analytics-bar">
      <div><span>{item[labelKey] || 'Unknown'}</span><strong>{formatNumber(value)}</strong></div>
...
export default function Analytics() {
  ...
  const refresh = () => getAdminAnalyticsOverview({ preset, force: 'true' })
    .then((nextData) => {
      setData(nextData)
  ...
  if (loading && !data) return <><div className="admin-page-heading"><div><span className="eyebrow">Customer insight</span><h1>Analytics</h1><p>Loading customer behaviour and GA4 reporting.</p></div></div><Loader label="Loading analytics" /></>

  return <>
    <div className="admin-page-heading">
      <div><span className="eyebrow">Customer insight</span><h1>Analytics</h1><p>Understand public website traffic, booking intent, service interest, and waitlist performance.</p></div>
      <div className="analytics-actions">
        ...
        <Button type="button" variant="outline" size="sm" onClick={refresh} loading={loading}><RefreshCcw size={15} /> Refresh</Button>
        {status?.gaUrl && <a className="button button--ghost button--sm" href={status.gaUrl} target="_blank" rel="noreferrer">Open GA4 <ExternalLink size={15} /></a>}
      </div>
    </div>
    ...
    {!disconnected && <div className="admin-cards analytics-cards">
      ...
      <article><span>New users</span><Comparison item={overview.newUsers} /></article>
