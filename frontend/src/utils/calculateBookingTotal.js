export function calculateBookingTotal(selectedServices = [], discount = 0) {
  ...
  const totalDuration = selectedServices.reduce((sum, service) => sum + Number(service.durationMinutes || 0), 0)
  const safeDiscount = Math.min(Math.max(Number(discount) || 0, 0), subtotal)
  return { subtotal, discount: safeDiscount, totalDuration, total: subtotal - safeDiscount }
}
