...
export default function ImagePlaceholder({ src, fallbackSrc, alt = '', category, label, variant = 'rounded', className = '', loading = 'lazy', fetchPriority, sizes, width, height }) {
  ...
  return <div className={`image-placeholder image-placeholder--${variant} ${loaded ? 'has-image' : ''} ${className}`.trim()}>
    {currentSrc && !failed && <img className="image-placeholder__image" src={currentSrc} alt={alt} loading={loading} ref={(node) => { if (node && fetchPriority) node.setAttribute('fetchpriority', fetchPriority) }} sizes={sizes} decoding="async" width={width} height={height} onLoad={() => setLoaded(true)} onError={handleError} />}
