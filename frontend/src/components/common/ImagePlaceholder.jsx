...
export default function ImagePlaceholder({ src, fallbackSrc, alt = '', category, label, variant = 'rounded', className = '', loading = 'lazy', fetchPriority, width, height }) {
  ...
  return <div className={`image-placeholder image-placeholder--${variant} ${loaded ? 'has-image' : ''} ${className}`.trim()}>
    {currentSrc && !failed && <img className="image-placeholder__image" src={currentSrc} alt={alt} loading={loading} fetchPriority={fetchPriority} decoding="async" width={width} height={height} onLoad={() => setLoaded(true)} onError={handleError} />}
