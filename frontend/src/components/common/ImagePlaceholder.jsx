...
export default function ImagePlaceholder({ src, fallbackSrc, alt = '', category, label, variant = 'rounded', className = '' }) {
  ...
  return <div className={`image-placeholder image-placeholder--${variant} ${loaded ? 'has-image' : ''} ${className}`.trim()}>
    {currentSrc && !failed && <img className="image-placeholder__image" src={currentSrc} alt={alt} onLoad={() => setLoaded(true)} onError={handleError} />}
