...
export default function PageHero({ eyebrow, title, subtitle, primaryCta, secondaryCta, tags = [], variant = 'split', image, imageAlt = '', imageCategory }) {
  return <section className={`page-hero page-hero--${variant}`}>
    ...
    {variant !== 'centered' && <div className="page-hero__visual"><ImagePlaceholder src={image || imagePaths.placeholders.hero} fallbackSrc={imagePaths.placeholders.hero} alt={imageAlt} category={imageCategory} variant={variant === 'editorial' ? 'editorial' : 'arch'} /></div>}
  </section>
}
